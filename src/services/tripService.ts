...
export async function getMyTrips(uid, limitCount = 20) {
  const q = query(
    collection(db, 'trips'),
    where('uid', '==', uid),
...
export async function addLocation(tripId, location) {
  const snap = await getDoc(doc(db, 'trips', tripId));
...
export async function searchByTag(tag, limitCount = 20) {
  const q = query(
    collection(db, 'trips'),
    where('isPublic', '==', true),
